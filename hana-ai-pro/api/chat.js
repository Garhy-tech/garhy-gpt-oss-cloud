...
export default async function handler(req, res) {
  ...
  if (Number.isFinite(contentLength) && contentLength > 32768) return res.status(413).json({ error: 'Request too large.' });
  ...
  try {
    ...
    if (!response.ok) {
      const fallback = response.status === 429
        ...
        : response.status === 401 || response.status === 403
          ...
          : 'Groq request failed.';
      console.error('Groq API error', { status: response.status, message: String(payload?.error?.message || '').slice(0, 300) });
      return res.status(response.status === 429 ? 429 : 502).json({ error: safeProviderMessage(payload, fallback) });
    }

    const providerReply = payload?.choices?.[0]?.message?.content;
    if (typeof providerReply !== 'string' || !providerReply.trim()) return res.status(502).json({ error: 'The model returned an empty response.' });
    ...
    console.error('Groq request error', { name: error?.name, message: String(error?.message || '').slice(0, 300) });
    return res.status(502).json({ error: isTimeout ? 'The model request timed out.' : 'Groq request failed.' });
  }
}
