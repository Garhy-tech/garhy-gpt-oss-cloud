...
function worker(){
  ...
  const context={URL,Request,Response,Set,console,fetch:async()=>{fetched++;return new Response('asset',{headers:{'Content-Type':'application/javascript'}});},caches:{open:async()=>cache,keys:async()=>['gt-bybit-shell-v3','gt-bybit-shell-20260908-2','other-app'],delete:async(key)=>deleted.push(key),match:async()=>undefined},self:{location:{origin:'https://bybit.garhy.tech'},addEventListener:(name,fn)=>handlers[name]=fn,skipWaiting:async()=>{},clients:{claim:async()=>{claimed=true;}}}};
